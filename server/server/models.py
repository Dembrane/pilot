from datetime import datetime
from typing import List, Set, Any
from sqlalchemy import (
    Column,
    ForeignKey,
    Table,
    create_engine,
    String,
    Text,
    Integer,
    DateTime,
    func,
)
from sqlalchemy.orm import (
    sessionmaker,
    mapped_column,
    Mapped,
    relationship,
    declarative_base,
)
from langchain_core.messages import AIMessage, HumanMessage, SystemMessage

from server.config import DATABASE_URL


# Create the engine and connect to the SQLite database file
engine = create_engine(DATABASE_URL)

# Create a session factory
Session = sessionmaker(bind=engine)

# Define your models as subclasses of the base class
Base: Any = declarative_base()


session_message_document_association_table = Table(
    "association_table",
    Base.metadata,
    Column("session_message_id", ForeignKey("session_message.id")),
    Column("document_id", ForeignKey("document.id")),
)


class SessionModel(Base):
    __tablename__ = "session"

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now()
    )
    updated_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now(), onupdate=func.now()
    )
    name: Mapped[str] = mapped_column(String, nullable=True)
    language: Mapped[str] = mapped_column(String, default="en")
    context: Mapped[str] = mapped_column(Text, nullable=True)
    documents: Mapped[List["DocumentModel"]] = relationship(
        "DocumentModel", back_populates="session"
    )
    messages: Mapped[List["SessionMessageModel"]] = relationship(
        "SessionMessageModel", back_populates="session"
    )
    processing_since: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True
    )


class SessionMessageModel(Base):
    __tablename__ = "session_message"

    id: Mapped[str] = mapped_column(String, primary_key=True)
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now()
    )
    text: Mapped[str] = mapped_column(Text)
    from_user: Mapped[bool] = mapped_column(Integer, default=False)
    session_id: Mapped[int] = mapped_column(Integer, ForeignKey("session.id"))
    session: Mapped["SessionModel"] = relationship(
        "SessionModel", back_populates="messages"
    )

    documents_used: Mapped[Set["DocumentModel"]] = relationship(
        "DocumentModel",
        secondary=session_message_document_association_table,
        back_populates="session_messages",
    )

    def get_lc_message(self) -> AIMessage | HumanMessage | SystemMessage:
        if self.from_user:
            return HumanMessage(content=self.text)
        else:
            return AIMessage(content=self.text)


class DocumentModel(Base):
    __tablename__ = "document"

    id: Mapped[str] = mapped_column(String, primary_key=True)
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now()
    )
    updated_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now(), onupdate=func.now()
    )
    session_id: Mapped[int] = mapped_column(Integer, ForeignKey("session.id"))
    session: Mapped["SessionModel"] = relationship(
        "SessionModel", back_populates="documents"
    )
    original_filename: Mapped[str] = mapped_column(String, default="")
    path: Mapped[str] = mapped_column(String)
    title: Mapped[str] = mapped_column(String, nullable=True)
    description: Mapped[str] = mapped_column(Text, nullable=True)
    context: Mapped[str] = mapped_column(Text, nullable=True)
    is_processed: Mapped[bool] = mapped_column(Integer, nullable=True, default=False)
    processing_error: Mapped[str] = mapped_column(Text, nullable=True)

    messages: Mapped[List["DocumentMessageModel"]] = relationship(
        "DocumentMessageModel", back_populates="document"
    )

    session_messages: Mapped[List["SessionMessageModel"]] = relationship(
        "SessionMessageModel",
        secondary=session_message_document_association_table,
        back_populates="documents_used",
    )


class DocumentMessageModel(Base):
    __tablename__ = "document_message"

    id: Mapped[str] = mapped_column(String, primary_key=True)
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now()
    )
    text: Mapped[str] = mapped_column(Text)
    from_user: Mapped[bool] = mapped_column(Integer, default=False)
    # for global question or answer
    is_global: Mapped[bool] = mapped_column(Integer, default=False)
    document_id: Mapped[str] = mapped_column(String, ForeignKey("document.id"))
    document: Mapped["DocumentModel"] = relationship(
        "DocumentModel", back_populates="messages"
    )

    # possibly overengineered
    def get_lc_message(self) -> AIMessage | HumanMessage | SystemMessage:
        if self.from_user:
            return HumanMessage(content=self.text)
        else:
            if self.is_global:
                return SystemMessage(content=self.text)
            return AIMessage(content=self.text)


Base.metadata.create_all(engine)

db = Session()
