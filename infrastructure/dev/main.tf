...
resource "azurerm_network_security_rule" "allow_from_public_subnet" {
...
resource "azurerm_network_security_rule" "allow_app_gateway_health_probes" {
  ...
  protocol                    = "*"
  source_port_range           = "*"
  destination_port_ranges     = ["8055", "8000", "5173"]
...
resource "azurerm_container_group" "directus" {
  ...
  container {
    ...
    environment_variables = {
      ...
      DB_HOST = "${azurerm_cosmosdb_postgresql_cluster.cosmo.name}.postgres.cosmos.azure.com"
