...
async def create_report(project_id: str, db: DependencyInjectDatabase, body: CreateReportRequestBodySchema) -> None:
	language = body.language or "en"
	try:
		report_content_response = await get_report_content_for_project(project_id, db, language)
	except ContextTooLongException:
		report = directus.create_item("project_report", item_data={
			"content": "",
			"project_id": project_id,
			"language": language,
			"status": "error",
