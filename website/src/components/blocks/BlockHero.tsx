...
const BlockHero: React.FC<BlockHeroProps> = async ({ block, lang }) => {
  ...
  return (
    <div className="relative flex h-full w-full flex-col justify-start px-2 py-4 md:flex-row md:px-4 md:py-8 lg:px-8 lg:py-16">
      <div
        className={`w-full justify-items-end md:w-1/2 ${blockData.image_position === 'left' ? 'md:order-first' : 'md:order-last'}`}
      >
      ...
      <div className="w-full space-y-4 text-center md:w-1/2 md:text-left">
        <AnimateLetters
          className="text-7xl md:-translate-x-[0.2vw] lg:text-[10vw]"
        ...
        <SlideIn>
          ...
          {blockData.button_group && <div className="mt-6">
